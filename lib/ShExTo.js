let ShExToOwl = function (schema, options = {}) {
  ...
  let untyped = {}
  let predicates = ShEx.Util.getPredicateUsage(schema, untyped, options.anyURIasDataProperty)
...
${rec.isAbstract ? 'ABSTRACT ' : ''}<span class="shape-name"><dfn>${pname(label)}</dfn></span>`,
  ...
  function run (renderer, markup, nestInlinableStructure, chattyOWL) {
    ...
    Array().push.apply(ret, Object.keys(schema.shapes).filter(
      shapeLabel => !shapeLabel.startsWith('_:')
    ).map(
      shapeLabel => renderShapeExpr(shapeLabel, schema.shapes[shapeLabel], renderer, markup)
    ).concat(curPackage
...
/*
    Array().push.apply(owlm, Object.keys(model.properties).map(
      propName => {
      ...
      classId => 'Class: ddi:' + classId + ' SubClassOf:\n' +
        model.classes[classId].properties.filter(
          ...
          propertyRecord => {
            ...
            let type = pname(model.properties[propName].uniformType[0])
...
*/
  ...
  function inlineable (label) {
    return label.startsWith('_:') // !! do actual test here (which will be redundant against ShEx.Util.nestShapes)
  }
  ...
  function ShExCSerializer (nestInlinableStructure) {
    ...
    function ShExCClass (label, shape, markup, abstract, restricts, force = false) {
      ...
      return (force
        ...
        (force || !getPackage(shape) ? '' : '\n' + markup.packageStr(getPackage(shape))) +
        (force || getAnnotations(shape, 'comment').length === 0 ? '' : '\n' + markup.comment(trimMarkdown(getAnnotations(shape, 'comment')[0])) + '^^mark:')
  ...
  function isObject (valueExpr) {
    ...
    return !(v.type === 'NodeConstraint' &&
             (v.datatype &&
              (v.datatype !== XSD + 'anyURI' || options.anyURIasDataProperty)
              || v.nodeKind === 'Literal'))
  }
