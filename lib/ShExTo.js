let ShExToOwl = function (schema, options = {}) {
  ...
  Object.keys(schema._index.shapeExprs).forEach(label => {
    let shapeExpr = ShEx.Util.skipDecl(schema._index.shapeExprs[label])
  ...
  Object.keys(schema._index.shapeExprs).forEach(referrer => {
    ...
    index(ShEx.Util.skipDecl(schema._index.shapeExprs[referrer]))
...
${abstract ? 'ABSTRACT ' : ''}<span class="shape-name"><dfn>${pname(label)}</dfn></span>`
        ...
        <Class abbreviatedIRI="${pname(superClass)}"/>
          ...
          (shapeHierarchy.parents[label] || []).find(
            supercl =>
              ...
              getPackage(shape) === getPackage(ShEx.Util.skipDecl(schema._index.shapeExprs[supercl]))
          )
            ? [] //       ... skip that package
            : [`    <SubClassOf>
        <Class abbreviatedIRI="${pname(label)}"/>
  ...
  function inlineable (label) {
    return label.startsWith('_:') && ShEx.Util.skipDecl(schema._index.shapeExprs[label]).type === 'Shape' // !! do actual test here (which will be redundant against ShEx.Util.nestShapes)
  }
