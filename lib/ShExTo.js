let ShExToOwl = function (schema, options = {}) {
  ...
  // !! get the shapeHierarchy from ShExUtil
  let shapeHierarchy = Hierarchy.create()
  Object.keys(schema._index.shapeExprs).forEach(label => {
    let shapeExpr = ShExUtil.skipDecl(schema._index.shapeExprs[label])
  ...
  let untyped = {}
  let predicates = ShExUtil.getPredicateUsage(schema, untyped)
  //    IRI->{ uses: [label], commonType: shapeExpr }
  if ('errors' in options) {
    options.errors.untyped = untyped
  }

  let nestables = ShExUtil.nestShapes(schema, {
    no: true,
  ...
  let referrers = {}
  Object.keys(schema._index.shapeExprs).forEach(referrer => {
    if (!(referrer in referrers)) {
      referrers[referrer] = [] // so we don't need guards for referrers.parents[X]
    }
    index(ShExUtil.skipDecl(schema._index.shapeExprs[referrer]))

    function index (shapeExpr) {
      if (shapeExpr.type === 'Shape') {
        let tcs = ShExUtil.simpleTripleConstraints(shapeExpr) || []
        tcs.forEach(tc => {
          let referree = ShExUtil.getValueType(tc.valueExpr)
          if (typeof referree === 'object') {
            index(referree)
          } else {
...
${abstract ? 'ABSTRACT ' : ''}<span class="shape-name"><dfn>${pname(label)}</dfn></span>`
  ...
  function run (serializer, markup, nestInlinableStructure, chattyOWL) {
    ...
    let curPackage = null
    Array().push.apply(ret, Object.keys(schema._index.shapeExprs).filter(
      shapeLabel => !inlineable(shapeLabel) // !shapeLabel.startsWith('_:')
    ).map(
      shapeLabel => renderShapeExpr(shapeLabel, schema._index.shapeExprs[shapeLabel], serializer, markup)
    ).concat(curPackage
  ...
  function OWLXMLSerializer (chatty) {
    ...
    function OWLXMLClass (label, shape, markup, abstract, restricts) {
      return []
        ...
        .concat(ShExUtil.simpleTripleConstraints(shape).map(
          tripleConstraint => {
            ...
            let type = ShExUtil.getValueType(tripleConstraint.valueExpr)
        ...
        <Class abbreviatedIRI="${pname(superClass)}"/>
          ...
          (shapeHierarchy.parents[label] || []).find(
            supercl =>
              ...
              getPackage(shape) === getPackage(ShExUtil.skipDecl(schema._index.shapeExprs[supercl]))
          )
            ? [] //       ... skip that package
            : [`    <SubClassOf>
        <Class abbreviatedIRI="${pname(label)}"/>
        ...
        <Literal datatypeIRI="http://www.w3.org/1999/02/22-rdf-syntax-ns#PlainLiteral">${encodeCharData(trimMarkdown(comment))}</Literal>
          ...
          (chatty ? ShExUtil.simpleTripleConstraints(shape).reduce(
    ...
    function OWLXMLProperty (predicate, type, markup) {
      ...
      let fakeRef = schema._index.shapeExprs[type] || {type: 'NodeConstraint', datatype: type} // forge a ShapeRef
  ...
  function inlineable (label) {
    return label.startsWith('_:') && ShExUtil.skipDecl(schema._index.shapeExprs[label]).type === 'Shape' // !! do actual test here (which will be redundant against ShExUtil.nestShapes)
  }
    ...
    function ShExCClass (label, shape, markup, abstract, restricts, force = false) {
      ...
      return (force
        ...
        (shape.closed
              ...
              : '') +
        (shape.extends || []).map(
          extension => ' EXTENDS @' + markup.reference(extension)
        ).join('') +
        ' {\n' +
        ShExUtil.simpleTripleConstraints(shape).map(
          tripleConstraint => {
            let propName = tripleConstraint.predicate
            let type = 'valueExpr' in tripleConstraint
                  ? ShExUtil.getValueType(tripleConstraint.valueExpr)
                  : '.'
            // if (type === undefined) {
            ...
            let valueStr = tripleConstraint.valueExpr.nodeKind
                  ...
                  ? indent(ShExCClass(type, ShExUtil.skipDecl(
                    typeof type === 'object' ? type : schema._index.shapeExprs[type]
                  ), markup, false, null, true), '  ')
                  : typeof tripleConstraint.valueExpr === 'string'
  ...
  function deref (valueExpr) {
    ...
    return typeof valueExpr === 'string'
      ? schema._index.shapeExprs[valueExpr]
      : valueExpr
  }
