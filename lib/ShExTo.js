let ShExToOwl = function (schema, options = {}) {
  ...
  Object.keys(schema.shapes).forEach(referrer => {
    ...
    index(ShEx.Util.skipDecl(schema.shapes[referrer]))
        ...
        <${t}MaxCardinality cardinality="${upper}">
          ...
          (shape.extends || []).map(
            superClass =>
              `    <SubClassOf>
        <Class abbreviatedIRI="${pname(label)}"/>
        <Class abbreviatedIRI="${pname(superClass.reference)}"/>
    </SubClassOf>`
          )
        ).concat(
          ...
          (shapeHierarchy.parents[label] || []).find(
            supercl =>
              ...
              getPackage(shape) === getPackage(ShEx.Util.skipDecl(schema.shapes[supercl.reference]))
          )
            ? [] //       ... skip that package
            : [`    <SubClassOf>
        <Class abbreviatedIRI="${pname(label)}"/>
    ...
    function ShExCClass (label, shape, markup, abstract, restricts, force = false) {
      ...
      return (force
        ...
        (shape.closed
              ...
              : '') +
        (shape.extends || []).map(
          extension => ' EXTENDS @' + markup.reference(extension.reference)
        ).join('') +
        ' {\n' +
        ShEx.Util.simpleTripleConstraints(shape).map(
          tripleConstraint => {
            ...
            let valueStr = tripleConstraint.valueExpr.nodeKind
                  ...
                  : tripleConstraint.valueExpr.type === 'ShapeRef'
