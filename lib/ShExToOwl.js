        ...
        (tcs || []).forEach(tc => {
          if (!(tc.predicate in predicates)) {
            ...
            if (curType === null) {
              ...
              ; // same type again
            } else if (shapeHierarchy[curType].parents.indexOf(newType) !== -1) {
              predicates[tc.predicate].polymorphic = true; // already covered by current commonType
            } else {
              let idx = shapeHierarchy[newType].parents.indexOf(curType)
              if (idx === -1) {
                console.warn(`${shapeLabel}/${tc.predicate}:${newType} uses untypable predicate`)
              } else {
                predicates[tc.predicate].commonType = shapeHierarchy[newType].parents[idx]
...
*/
    ...
    function OWLXMLClass (label, shape, markup, abstract, restricts) {
      return []
        ...
        .concat(simpleTripleConstraints(shape).map(
          tripleConstraint => {
            ...
            if (!('valueExpr' in tripleConstraint)) {
              console.warn(`skipping ${label}'s ${propName} with no valueExpr -- can't know if it's an object or data property`)
              return ''
            }
        ...
        <${t}MaxCardinality cardinality="${upper}">
          ...
          // subclass membership
          (shapeHierarchy.parents[label] || []).map(
            superClass =>
              `    <SubClassOf>
        <Class abbreviatedIRI="${pname(label)}"/>
  ...
  function isObject (valueExpr) {
    ...
    return !(v.type === 'NodeConstraint' && v.datatype/* !! uncomment && v.datatype !== XSD + "anyURI" */)
  }
